...
//       const response = await requestInventory(token);
...
//           const retryResponse =
//             await requestInventory(freshToken);

//           return retryResponse.data;
//         } catch (retryError) {
//           throw this.toInventoryException(retryError);
//         }
//       }

//       throw this.toInventoryException(error);
//     }
//   }
...
//   private toInventoryException(error: unknown) {
//     const status = this.getStatus(error);
...
//       return new BadGatewayException(
//         "Kira inventory request timed out.",
//       );
//     }

//     return new BadGatewayException(
//       "Kira inventory is temporarily unavailable.",
//     );
//   }
// }
...
export class KiraService {
  ...
  /*
   * Full CSV inventory.
   * Kira documentation shows:
   * pagestart=1&pageend=166
   */
  ...
  /*
   * Lightweight current availability list.
   */
  async getAvailableStockLimited():
    Promise<AvailableKiraStone[]> {
    try {
      return await this.withTokenRetry(
        async (token) => {
          const response =
  ...
  /*
   * Retained for direct debugging.
   * Website search will no longer use it.
   */
